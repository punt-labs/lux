...
timing hiccup on a loaded runner surfaces for visibility but **never blocks a
merge**. The default `addopts` filter is untouched; each job overrides the marker
via its Make target, exactly as they do locally.
