"""TreeSelectionModel — raw construction, immutable per-mode verbs, serialize.

Mirrors ``tests/test_table_selection_model.py`` exactly, substituting node ids
for row ids — ``TreeSelectionModel`` is a structural copy of
``TableSelectionModel`` (see ``tree_selection_model.py``).
"""
...
class TestConstructionIsRaw:
    def test_holds_the_state_verbatim_for_the_validate_gate(self) -> None:
        ...
        assert model.selected_node_ids == frozenset({"a", "b"})
        assert model.anchor == "a"

    def test_is_selectable_reflects_mode(self) -> None:
        assert TreeSelectionModel(mode="none").is_selectable is False
        assert TreeSelectionModel(mode="single").is_selectable is True
        assert TreeSelectionModel(mode="multi").is_selectable is True
...
class TestWithSelection:
    def test_returns_a_new_instance_leaving_the_original_untouched(self) -> None:
        ...
        original = TreeSelectionModel(mode="multi")
        updated = original.with_selection(frozenset({"a", "b"}))
        assert updated is not original
        assert original.selected_node_ids == frozenset()
        assert updated.selected_node_ids == frozenset({"a", "b"})

    def test_none_mode_stays_empty(self) -> None:
        model = TreeSelectionModel(mode="none").with_selection(frozenset({"a", "b"}))
        assert model.selected_node_ids == frozenset()
        assert model.anchor == ""

    def test_single_mode_keeps_one(self) -> None:
        model = TreeSelectionModel(mode="single").with_selection(
            frozenset({"a", "b", "c"})
        )
        assert len(model.selected_node_ids) == 1

    def test_multi_mode_keeps_the_whole_set(self) -> None:
        model = TreeSelectionModel(mode="multi").with_selection(
            frozenset({"a", "b", "c"})
        )
        assert model.selected_node_ids == frozenset({"a", "b", "c"})

    def test_anchor_reseats_onto_a_selected_node(self) -> None:
        model = TreeSelectionModel(mode="multi", anchor="z").with_selection(
            frozenset({"a", "b"})
        )
...
class TestWithAnchor:
    ...
    def test_drops_an_anchor_outside_the_selection(self) -> None:
        ...
        assert model.anchor == "a"  # reseated to a selected node
...
class TestReconciled:
    def test_drops_departed_ids(self) -> None:
        ...
        assert model.selected_node_ids == frozenset({"a", "c"})
    ...
    def test_departed_anchor_reseats_to_a_survivor(self) -> None:
        ...
        assert model.selected_node_ids == frozenset({"a"})
        assert model.anchor == "a"
