"""FrameOps -- the Protocol every frame_close implementer satisfies structurally."""
...
def test_an_object_missing_close_frame_does_not_satisfy_it() -> None:
    assert not isinstance(object(), FrameOps)


def test_close_frame_takes_a_plain_frame_id_not_a_value_object() -> None:
    ...
    result = _Implementer().close_frame("board", scope=Scope(ConnectionId("c1")))
    assert result == Ok()
