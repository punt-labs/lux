...
from punt_lux.operations import Ok, OpError, Scope, SetMenuRequest
from tests.commands._family_stubs import StubMenuOps
from tests.commands._scene_stub import identity

_SCOPE = Scope(ConnectionId("menu-set-test"))
...
def test_success_renders_ok() -> None:
    ...
    result = asyncio.run(menu_set(ctx, _request(), scope=_SCOPE))
...
def test_execute_returns_the_typed_outcome_with_no_envelope() -> None:
    ...
    result = asyncio.run(menu_set.execute(ctx, _request(), scope=_SCOPE))
...
def test_routes_request_through_to_ops() -> None:
    ...
    asyncio.run(menu_set.execute(ctx, request, scope=_SCOPE))
