...
An unidentified fd has no attribution to install content under and is
rejected uniformly, the same invariant
:class:`~punt_lux.display.identity_guard.IdentityGuard` enforces for scenes.
"""
...
class ContentMessageGate:
    ...
    def handle_agent_menus(self, sock: socket.socket, msg: MenuMessage) -> None:
        ...
        if not self._identity.reject_if_unidentified(sock, type(msg).__name__):
            hub = self._identity.hub_id_of(sock)
            self._menus.replace_agent_menus(msg.menus, hub)
    ...
    def handle_callback_menus(
        ...
        if not self._identity.reject_if_unidentified(sock, type(msg).__name__):
            hub = self._identity.hub_id_of(sock)
            self._menus.replace_callback_menus(msg.submenus, hub)
