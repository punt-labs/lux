...
``end`` open and close the tab-bar surface, and ``begin_tab`` / ``end_tab``
bracket each tab. ``begin_tab`` does the two-way work: it HONOURS the
Hub-authoritative active tab (force-selecting the matching tab the frame a fresh
Hub value arrives) and FIRES ``tab_changed`` on a genuine user switch.

Echo-suppression: the honoured value is recorded per-scene in ``WidgetState``.
On the frame a Hub-driven change arrives (``active`` differs from the last
honoured value), the adapter force-selects and does NOT fire — so a Hub-driven
change never re-fires and no fire -> Hub -> re-push -> fire loop can run. A user
switch (``active`` unchanged since last frame, but a different tab reports
selected) fires exactly once.
"""
...
class ImGuiTabBarRenderer:
    ...
    def end(self, *, opened: bool) -> None:
        """Close the tab-bar surface and record the honoured active tab.

        Recording once per frame (not per tab) keeps the last-honoured value
        stable across the frame's ``begin_tab`` calls, so the fire decision
        reads a consistent baseline.
        """
        if opened:
            imgui.end_tab_bar()
    ...
    def begin_tab(self, tab: Tab, *, active: str) -> bool:
        ...
        last = self._factory.widget_state.get(self._honoured_key(), active)
    ...
    def _select_flags(tab_id: str, active: str, last: str) -> int:
        """Force-select this tab only when the Hub active tab changed this frame."""
    ...
    def _is_user_switch(*, selected: bool, tab_id: str, active: str, last: str) -> bool:
        ...
        A tab that is not the reported selection, or is already the active tab,
        is no switch. A frame that honoured a fresh Hub value (``active`` differs
        from the last-honoured ``last``) is the echo, not a user act — so it does
        not fire (echo-suppression).
        """
