...
class FrameRaiseCommand:
    ...
    async def execute(self, ctx: Ctx[FrameOps], frame_id: str) -> FrameRaise | OpError:
        """Raise ``frame_id`` and return the typed outcome."""
        return await asyncio.to_thread(ctx.ops.raise_frame, frame_id)
