...
if TYPE_CHECKING:
    ...
    from punt_lux.operations import DisplayStateSnapshot
...
class DisplayStateGetCommand:
    ...
    async def execute(
        self, ctx: Ctx[DisplayStateOps]
    ) -> DisplayStateSnapshot | OpError:
        """Return the typed snapshot, or the display's fault."""
        return await asyncio.to_thread(ctx.ops.get_display_state)

    async def __call__(self, ctx: Ctx[DisplayStateOps]) -> CommandResult:
