...
three parameters threaded down each path.

It is also what keeps the two states in :mod:`punt_lux.applets.board_cache` from
reaching through a load to a client: they are handed the work and tell it what to
do, rather than being handed its parts and doing the work themselves.
"""
...
class BoardWork:
    ...
    def answered_with(self, note: str) -> None:
        """Say what this click's visible answer was, on the line reporting it."""
        self._latency.answered_with(note)
    ...
    def issues(self) -> BeadsRows:
        """Read the issues, or raise the reason they could not be read."""
