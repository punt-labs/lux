"""Standalone ``HandlerDecoder[ValueChanged]`` builder for ComboElement.

Parallel to ``standalone_checkbox_handler.py``: a combo without a parent
...
(``_UpdateSelectedHandler``) is installed separately by ``JsonComboDecoder``
...
__all__ = ["build_standalone_combo_handler_decoder"]
...
def build_standalone_combo_handler_decoder(
    sink: PublishSink,
) -> HandlerDecoder[ValueChanged]:
    """Return the ``HandlerDecoder`` for a standalone ComboElement.

    Registers one explicit factory:
    - ``noop``: do-nothing handler (used when the only side effect is a
      decorator like ``publish``)

    The built-in state-sync handler is installed directly by ``JsonComboDecoder``
    so every decoded combo keeps its selected index mirrored even when the wire
