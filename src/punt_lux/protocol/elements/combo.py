"""ComboElement — a single-selection dropdown on the Element ABC.
...
rebinds the real factory. The codec body lives in ``combo_codec.py``; ``to_dict``
...
@final
class ComboElement(Element):
    """A single-selection dropdown on the Element ABC.
    ...
    @property
    def id(self) -> str:
        """Return the combo's stable identity within its enclosing Scene."""
        return self._id

    @property
    def kind(self) -> Literal["combo"]:
        """Return the wire discriminator — always ``"combo"``."""
        return self._kind

    @property
    def label(self) -> str:
        """Return the visible combo label."""
        return self._label
    ...
    def _index_error_messages(self) -> tuple[str, ...]:
        ...
        address a real item; with no items only ``0`` is meaningful (a combo
        awaiting deferred population). Reporting — not the legacy silent
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return JsonComboEncoder().encode(self)

    @classmethod
    def from_dict(cls, d: Mapping[str, object]) -> ComboElement:
        ...
        other. Wires a noop-only handler decoder so a combo with no ``handlers``
        decodes without a publish bus; a ``publish`` chain raises via
        ``RaisingPublishSink``.
        """
        decoder = JsonComboDecoder(
