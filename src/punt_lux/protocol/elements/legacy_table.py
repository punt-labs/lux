"""Legacy table dataclass — the pre-ABC fat table, retained until B7.

``LegacyTableElement`` keeps the old wire shape (``filters`` + ``detail`` and its
``TableFilter`` / ``TableDetail`` value classes) so the legacy renderer and any
not-yet-cut-over decode path still work while the ABC ``TableElement`` (the basic
grid) takes the canonical name. Deleted with the legacy render path in B7.
"""
...
class TableDetail:
    ...
    Each array is parallel to the parent ``LegacyTableElement.rows``:
...
class LegacyTableElement:
    """A data table with columns and rows (legacy fat form, retained until B7)."""
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> Self:
        """Construct a LegacyTableElement from a JSON-decoded mapping."""
...
def register_codecs(register: Register) -> None:
    ...
    register(
        "table",
        LegacyTableElement,
        LegacyTableElement.to_dict,
        LegacyTableElement.from_dict,
    )
