...
``_UpdateTextHandler`` that mirrors the authoritative text on every
...
class _UpdateTextHandler:
    ...
    the authoritative text via ``apply_patch``. On the Display side,
    ...
    def __new__(cls, elem: InputTextElement) -> Self:
...
class JsonInputTextDecoder:
    ...
    def __new__(
        ...
        *,
        ...
        emit: Emit,
        element_cls: type[InputTextElement],
    ...
    @trace
    def decode(self, raw: Mapping[str, object]) -> InputTextElement:
    ...
    def _install_handlers(
        self, elem: InputTextElement, raw: Mapping[str, object]
    ) -> None:
        ...
        if not isinstance(handlers_raw, list):
            msg = (
                "input_text 'handlers' must be a list, "
        ...
        for i, spec in enumerate(cast("list[object]", handlers_raw)):
            if not isinstance(spec, dict):
                msg = (
                    f"input_text 'handlers[{i}]' must be a mapping, "
    ...
    def _resolve_event_type(
        ...
        if not isinstance(event_name, str) or not event_name:
            msg = (
                f"input_text 'handlers[{index}]' requires an 'event' string, "
                f"got {event_name!r}"
            )
            raise ValueError(msg)
        event_type = _INPUT_TEXT_EVENT_TYPES.get(event_name)
        if event_type is None:
            known = sorted(_INPUT_TEXT_EVENT_TYPES)
            msg = (
                f"input_text 'handlers[{index}].event' = {event_name!r} is not "
...
class JsonInputTextEncoder:
    ...
    def encode(self, elem: InputTextElement) -> dict[str, object]:
        """Serialize an InputTextElement to a JSON-compatible dict."""
        d: dict[str, object] = {
            "kind": elem.kind,
            "id": elem.id,
            "label": elem.label,
