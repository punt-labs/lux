"""Standalone ``HandlerDecoder[ValueChanged]`` builder for RadioElement.

Parallel to ``standalone_checkbox_handler.py``: a radio without a parent
...
(``_UpdateSelectedHandler``) is installed separately by ``JsonRadioDecoder``
before any wire-declared handlers are decoded. The decorator registry binds to
whatever ``PublishSink`` the caller supplies.
"""
...
__all__ = ["build_standalone_radio_handler_decoder"]
...
def build_standalone_radio_handler_decoder(
    sink: PublishSink,
) -> HandlerDecoder[ValueChanged]:
    """Return the ``HandlerDecoder`` for a standalone RadioElement.

    Registers one explicit factory:
    - ``noop``: do-nothing handler (used when the only side effect is a
      decorator like ``publish``)

    The built-in state-sync handler is installed directly by ``JsonRadioDecoder``
    so every decoded radio keeps its selected index mirrored even when the wire
