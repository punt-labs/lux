...
deaths, when the most recent one landed, and, when the Display managed to report
one, the render error text. The record is owned by the store's scene entry
...
class QuarantineRecord:
    ...
    # dying; attribution never depends on one arriving (display-crash-quarantine.md
    # Question 1 — a crash unwinds the frame with no reliable flush point).
    ...
    def describe(self, scene_id: str) -> str:
        ...
        if self.render_error is None:
            return base
        return f"{base}; last reported render error: {self.render_error}"
