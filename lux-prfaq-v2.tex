...
developer's own machine; Lux~v2 gives them a native visual surface built on
...
or terminal scrollback that has no structure at all, Lux~v2 extends itself:
...
both: the GPU-accelerated ImGui/SDL2 display from v1, plus the
self-extension model from Pharo.  Terminal
agents are the most powerful agentic tools available, but they have no visual
surface.  Web dashboards provide visuals but pull your agent onto vendor
compute.  Lux fills that gap --- native rendering, local-first, and the
result is that `Lux can't do X' stops being a limitation and becomes a
five-minute problem.''
...
\begin{faqpair}{Who are the competitors and why will we win?}\label{faq:competitors}
  ...
  \begin{itemize}[nosep]
    ...
    \item \textbf{Claude Desktop} --- Produces novel visual interfaces
      conceptually similar to the self-extending display we describe.
...
\begin{faqpair}{Why now?  What has changed?}
  ...
  (4)~Lux~v1 proved the display surface has value and the
...
Value      & \textcolor{RiskAmber}{Medium} &
  ...
  users, and whether the ``visual surface for terminal agents without
  giving up local compute'' positioning lands with developers who currently
  accept text-only output. \\
...
Feasibility & \textcolor{RiskAmber}{Medium} &
  ...
  work.  Outside view: single-engineer 6-phase rewrites with new
  extension systems typically underestimate scope.  Mitigated by
  phased delivery with a defined cut line after phase~3. \\
