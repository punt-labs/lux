...
|--------|-------------|---------|--------|
| **A — Horizontal Bands** | One architectural concept per PR across the whole codebase. ~7 PRs, each cross-cutting. | REJECTED (3/3 ITERATE) | PR 6 in this method ("all 24 kinds migrated and old paths deleted in one PR") is a flag day. Empty infrastructure (PR 2) ships before any consumer. Shadow Display tax has no production value. If PR 6 is split family-by-family, A collapses into B. |
