...
%                                    cleanup_stale, reap
%   src/punt_lux/display/socket_server.py -- SocketListener.setup: the
...
\texttt{CleanupStale}, \texttt{Bind}'s pre-condition path, \texttt{Reap},
and \texttt{ReapDead}.  Their guards are, respectively,
\texttt{socketStatus} $\in$ \texttt{\{sstale, sbound\}}; a start from
\texttt{sabsent}; \texttt{socketStatus = slistening}; and
\texttt{socketStatus} $\neq$ \texttt{slistening}.  Of
