%
% commit_on_idle_reconciliation.tex
%
% THE general Z specification of lux's commit-on-idle reconciliation
% discipline: the honour / defer / commit-echo state machine that governs
% EVERY non-atomic mutable (continuous-edit) element -- input_text, slider,
% input_number, color_picker -- through the ONE shared ContinuousEditArbiter.
% It is the buffer/editing/commit discipline that decides, on a Hub/Display
% round trip with a DELAYED echo, when the local buffer defers the Hub value
% and when it honours it.
%
% The model was first extracted from input_text, where two reconciliation-
% latency defects surfaced across two review rounds -- the pipelined-edits
% clobber (which motivated the commit-on-idle redesign) and the residual
% ``refocus loses commit echo'' (Copilot MEDIUM) -- but it is not an input_text
% model.  It is data-independent: its carrier [VALUE] is a parameter, not a
% concrete type, and the same state machine governs all four arbiters unchanged
% under three valid instantiations -- [VALUE] := str (the text an input_text
% holds), [VALUE] := float (the number a slider or an input_number holds), and
% [VALUE] := RGBA-tuple (the arity-4 float tuple a color_picker holds).  A
% specification that governs four components is named for none of them: hence
% commit_on_idle, not input_text.
%
% The two atomic-selection kinds are explicitly OUT of scope.  Checkbox, combo,
% radio, and selectable hold no editable buffer, open no echo-latency window,
% carry no arbiter, and need no reconciliation: an atomic selection commits in
% one step and honours the Hub every frame, so this discipline does not apply.
%
% The input_number reuses the slider's float carrier verbatim (the
% integer variant is a float-representable value, coerced to int only at the
% input_int widget seam), so it adds no new carrier and no new interleaving.  The
% tuple
% adds no new interleaving: a color is atomic at the wire (one hex string), the
% widget (one changed/deactivate signal), the commit (one ValueChanged), and the
% echo (one atomic apply_patch), so there is no partial-echo or per-component
% race for the model to miss.  The one type-specific proof obligation -- tuple ==
% is reflexive iff no component is NaN -- is discharged STRUCTURALLY for the tuple
% carrier: the wire value is a hex string, which cannot encode NaN/inf, and the
% hex->float parse is total onto finite [0,1].  This is stronger than the slider,
% whose float carrier needed a math.isfinite guard in validate() because raw
% floats crossed the wire.
%
% Source of record:
%   src/punt_lux/display/renderers/imgui/continuous_edit_selection.py
%       -- ContinuousEditArbiter[T]: the ONE shared implementation of this state
%          machine for all three carriers -- resolve (honour-or-defer), observe,
%          commit, release. The two carrier-typed touches (buffer read, committed
%          coerce) are delegated to an injected ValueAccessor[T].
%   src/punt_lux/display/renderers/imgui/continuous_edit_accessors.py
%       -- StrValueAccessor / FloatValueAccessor / ColorValueAccessor: the per-type
%          buffer read (with its miss policy) and committed coercion. The color
%          carrier's finiteness precondition is discharged structurally by the hex
%          encoding; the float carrier's by SliderElement.validate and
%          InputNumberElement.validate (both math.isfinite, since raw numbers cross
%          the wire). The one FloatValueAccessor serves both float kinds unchanged.
...
%       -- InputNumberRenderer.render: the per-frame active/deactivate/fire
%          decision over an ImGui input_float / input_int widget, reusing the
%          FloatValueAccessor; the two commit conditions (deactivate-after-edit,
%          discrete stepper change) feed one commit so a gesture never double-fires
%   src/punt_lux/display/renderers/color_picker_renderer.py
%       -- ColorPickerRenderer.render: the per-frame active/deactivate/fire
%          decision over ImGui color_edit3/4 / color_picker3/4 widgets, committing
%          the quantized tuple so committed bit-equals the echo
%   src/punt_lux/display/replica/widget_state.py
%       -- WidgetState: the per-scene shared CONTINUOUS_EDIT_* buffer/editing/
%          commit-echo quad and its removal clear (discard_for)
%
% Type-check:  fuzz -t docs/commit_on_idle_reconciliation.tex
% Model-check (see Section "Verification" for the exact goal predicates):
%   probcli docs/commit_on_idle_reconciliation.tex -model_check -p DEFAULT_SETSIZE 2
%   probcli docs/commit_on_idle_reconciliation.tex -model_check -nodead \
...
\title{The lux Commit-on-Idle Reconciliation Discipline: a Z Specification}
...
This is the Z model of lux's \emph{commit-on-idle} reconciliation discipline:
the honour / defer / commit-echo state machine that every non-atomic mutable
element shares.  A non-atomic mutable --- a \emph{continuous-edit} --- element is
one whose value the user changes through a sustained gesture rather than a
single click: \texttt{input\_text}, \texttt{slider}, \texttt{input\_number}, and
\texttt{color\_picker}.  All four are reconciled through the one shared
\texttt{ContinuousEditArbiter}, and this specification governs all four
unchanged.  It does so because it is \emph{data-independent}: its only carrier,
\texttt{VALUE} (Section~``Basic Types''), is a parameter, not a concrete type,
and the machine never inspects the internal structure of a value --- only
whether two values are equal.  The carrier has three valid instantiations, one
per carried type: the text a \texttt{input\_text} holds
(\texttt{VALUE}~$:=$~\texttt{str}), the number a \texttt{slider} or an
\texttt{input\_number} holds (\texttt{VALUE}~$:=$~\texttt{float}), and the
arity-4 RGBA float tuple a \texttt{color\_picker} holds
(\texttt{VALUE}~$:=$~tuple).  The two \emph{atomic-selection} kinds ---
\texttt{checkbox}, \texttt{combo}, \texttt{radio}, and \texttt{selectable} ---
are out of scope (Section~\ref{sec:scope}): an atomic selection commits in one
step and honours the Hub every frame, so it has no buffer, no echo-latency
window, no arbiter, and nothing to reconcile.

The exposition below is cast in terms of \texttt{input\_text}, the element the
model was first extracted from and the canonical instantiation
(\texttt{VALUE}~$:=$~\texttt{str}).  Nothing in the argument depends on the value
being text: read ``text'' as ``value'' and the same reasoning holds for the
float and tuple carriers.  Section~``Basic Types'' records why the two other
carriers add no new interleaving for the model to miss.
...
\texttt{VALUE} is the set of values the field may hold.  It is a \emph{given
set} --- a parameter, not a concrete type --- and that is the whole of the
model's generality: the machine compares values for equality and never inspects
their structure, so the same schemas govern the \texttt{str} carrier of an
\texttt{input\_text}, the \texttt{float} carrier of a \texttt{slider} or an
\texttt{input\_number}, and the RGBA-tuple carrier of a \texttt{color\_picker}
without change.  The other two carriers add no new interleaving for the model to
miss.  The float carrier is a single value crossing the wire exactly as text
does; the one type-specific proof obligation --- that value equality is
reflexive, which fails for a \texttt{NaN} --- is discharged outside the model by
the elements' \texttt{validate} guards (\texttt{math.isfinite}) and, for the
tuple, structurally by its hex-string wire encoding, which cannot represent
\texttt{NaN}.  The tuple carrier is atomic at the wire (one hex string), the
widget (one changed/deactivate signal), the commit (one \texttt{ValueChanged}),
and the echo (one apply), so it exhibits no partial-echo or per-component race
absent from the text case.

Two values suffice to exhibit the loss: an initial and pre-echo value, and a
distinct committed value whose echo has not yet arrived.  Three exercise the
interleaving of a second edit against an in-flight echo more thoroughly.  The
carrier is bounded by ProB's \texttt{DEFAULT\_SETSIZE}, so no explicit
cardinality constant is needed.
...
Concretely, in \texttt{ContinuousEditArbiter}
(\texttt{continuous\_edit\_selection.py}):
