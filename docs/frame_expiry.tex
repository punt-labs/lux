...
removes the roots (\texttt{Expire}, \texttt{ManualClose}, \texttt{Reclaim}) clears
...
manual close it is a removal, so it tears the roots down and disarms the deadline
...
once the frame empties. It is a distinct removal path from \texttt{ManualClose} and
\texttt{Expire}, and the invariant is that \emph{every} removal path disarms: a
...
each of \texttt{Expire}, \texttt{ManualClose}, and \texttt{Reclaim} clears the
deadline as it clears the roots, and dropping the disarm from any one of them
reaches $B$.
