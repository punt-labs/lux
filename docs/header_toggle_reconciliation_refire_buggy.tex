%
% header_toggle_reconciliation_refire_buggy.tex
%
% FIDELITY CONTROL for header_toggle_reconciliation.tex -- NOT the design.
%
% This is the specification with exactly one guard removed, so that the defect
% the design closes becomes reachable.  It models a fix that renders the pending value but still decides to fire by
% comparing what ImGui reports against the raw Hub value.
%
% The edit:
% %   FrameQuiet fires when the rendered state differs from hub, instead of
%   staying silent -- the one guard removed.  FrameClick is untouched.
%
% Must find:
% %   probcli header_toggle_reconciliation_refire_buggy.tex -model_check \
%     -nodead -goal "fires>1" -p DEFAULT_SETSIZE 2
%
...
\title{The lux Collapsing-Header Toggle Discipline: Fidelity Control (c), Firing Against the Hub Value}
...
\quad~  \ELSE (\IF steps < MAXSTEP \THEN steps + 1 \ELSE MAXSTEP) \\
  ...
  pending' = pending \\
  clicked' = clicked \\
  fires' = \IF shown' = hub \THEN fires \\
\quad~  \ELSE (\IF fires < MAXFIRE \THEN fires + 1 \ELSE MAXFIRE) \\
  spurious' = spurious
\end{schema}
...
did.  Goal found: invariant~1 (\texttt{clicked=ztrue \& steps>1}).  Trace:
\begin{enumerate}
...
Under the intact definition, step~3 writes the pending \texttt{ztrue}, so \texttt{shown} does not move and \texttt{steps} stays 1.
...
speaks.  Goals found: invariant~4 (\texttt{pending/=nopending \&
 clicked=zfalse})
and invariant~5 (\texttt{rendered=ztrue \& clicked=zfalse \& shown/=hub}).
Trace:
\begin{enumerate}
...
which is a rejection correctly rendered.

\paragraph{Defect (c) --- firing against the Hub value rather than the
effective one.}
Edit: \texttt{FrameClick}'s guard becomes \texttt{shown' $\neq$ hub} and its
\texttt{fires'} increments rather than resetting, modelling a renderer that
keeps the pending slot for rendering but compares the reported state against
\texttt{elem.open} when deciding to fire.  Goal found: invariant~2
(\texttt{fires>1}).  Trace: the click frame fires once with
\texttt{shown = ztrue} against \texttt{hub = zfalse}; the next frame renders
the pending \texttt{ztrue}, still finds it differs from \texttt{hub = zfalse},
and fires again --- and so does every frame until the re-push, a fire storm
across the latency window.  Under the intact guard the comparison is against
the effective value, which the pending slot has moved to \texttt{ztrue}, so the
following frames find no difference and stay silent.
