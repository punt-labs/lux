...
%
% FIDELITY NEGATIVE CONTROL #1 for display_crash_loop.tex: the QUARANTINE
% clause struck.  This is the canonical corrected model with one change --
% BatchCrash and IsolCrash tally the death but never quarantine the scene
% (quarantined stays empty).  With no quarantine, IsolCrash fires forever
% against the same crasher and the crash-respawn loop is unbounded.  ProB
% reaches "crashes > 1 + (THRESHOLD-1)*card(crasher)" -- the goal that is NOT
% found for the fixed design.  This is the deterministic loop the quarantine
% rule prevents.
%
% Do NOT treat this as a design artifact; the canonical, verified model is
% display_crash_loop.tex.  See also display_crash_loop_earlyexit_buggy.tex,
% the control for the stable-interval exit.
%
...
\title{Crash-Loop Quarantine: Fidelity Control \#1 (quarantine struck)}
\author{Buggy variant --- no scene is ever quarantined}
\date{July 2026}
\maketitle
...
Type-checking is a \texttt{fuzz} pass on this variant:
\begin{verbatim}
  fuzz -t docs/display_crash_loop_buggy.tex
\end{verbatim}

This file is a fidelity negative control, so its verification expectation is
the \emph{inverse} of the canonical spec's: the crash-bound goal that is NOT
found for \texttt{display\_crash\_loop.tex} must be FOUND here, with the
deterministic crash-respawn loop as the counter-example trace:
\begin{verbatim}
  # Buggy variant (must report: goal FOUND -- the loop)
  probcli docs/display_crash_loop_buggy.tex -model_check -nodead \
    -goal "crashes>1+(THRESHOLD-1)*card(crasher)" \
    -p DEFAULT_SETSIZE 2 -p MAX_INITIALISATIONS 20
\end{verbatim}

The canonical spec's stable-state goal is unreachable here by construction:
with the quarantine clause struck, \texttt{quarantined} never grows, so a
non-empty \texttt{crasher} can never become a subset of it.  The directed
search above, which halts the instant the loop overshoots the bound, is the
runnable control.

The \texttt{-nodead} flag and the raised \texttt{MAX\_INITIALISATIONS} bound
carry over from the canonical spec: the search is goal-directed, and
\texttt{Init} is nondeterministic over \texttt{crasher}, so every initial
valuation must be explored.
