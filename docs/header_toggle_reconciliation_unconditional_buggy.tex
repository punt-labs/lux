%
% header_toggle_reconciliation_unconditional_buggy.tex
%
...
% the design closes becomes reachable.  It models the shipped renderer, which called set_next_item_open(elem.open)
% on every frame regardless of any toggle in flight.
%
% The edit:
% %   effective(h, p) = h for every p -- the pending value is never rendered.
%
% Must find:
% %   probcli header_toggle_reconciliation_unconditional_buggy.tex -model_check \
%     -nodead -goal "clicked=ztrue & steps>1" -p DEFAULT_SETSIZE 2
%
...
\title{The lux Collapsing-Header Toggle Discipline: Fidelity Control (a), the Unconditional Write}
...
\where
  \forall h : ZBOOL; p : PENDING @ effective(h, p) = h
\end{axdef}
