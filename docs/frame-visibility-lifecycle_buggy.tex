...
% SetDockState is the third restored defect: unguarded here, so a client can
% undock (or dock) a frame the user closed; guarded in the intact document.
%
...
% -------------------------------------------------------------------
\subsection{Set the Dock State --- unguarded}
% -------------------------------------------------------------------

The third restored defect, and the one the first version of this pair could not
see at all, because neither document carried the operation.  ``set\_frame\_state''
is on the whole client surface, and here it writes any visibility to any frame:
no guard on the frame's current state, and no guard on the value asked for.  So
a client holding a frame id can undock a frame the user closed --- putting the
window back with no gesture behind it --- or dock one, giving it a pill in the
bar the user never asked for.

The intact specification guards both ends: the frame must already be up or
docked, and the value must be one of those two.  Invariant~4 is what separates
them, and this operation is the one that reaches it without going near a push.
