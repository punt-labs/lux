...
\subsection{TimedReap --- the lease as a real background backstop}
\label{sec:timedreap}
...
\texttt{TimedReap} is the same atomic deregister-and-release predicate as
\texttt{Depart}, but with \texttt{Depart}'s permissive guard narrowed to
the one condition a background timer can act on without any other
party's help: the lease has lapsed. No other connection needs to exist,
act, or have ever existed, for \texttt{TimedReap(c?)} to become and stay
enabled.
...
The predicate is line-for-line \texttt{Depart}'s, with one added
conjunct in the guard. That repetition is deliberate, not an oversight:
Section~\ref{sec:inv}'s two-lock argument depends on every
registry-mutating operation sharing exactly this shape, and stating
\texttt{TimedReap} as ``\texttt{Depart} restricted to a lapsed lease''
in prose, without restating its predicate in full, would leave a reader
unable to check by inspection that the restriction is the \emph{only}
difference. \texttt{TimedReap} is, in fact, round 1's original
\texttt{Reap} operation, reinstated verbatim: round 1 was right that a
lapsed lease alone should be enough to trigger release, and this
addendum's ruling is that round 1 was also right to make it a
freestanding, unconditional operation rather than something this round's
first cut folded away entirely into \texttt{Depart} and \texttt{Claim}.
The difference from round 1 is only what it now stands \emph{beside}: an
explicit non-destructive \texttt{Read}, an explicit external-signal
\texttt{Depart}, and a \texttt{Claim} that also renews and sweeps -- so
the model no longer has to choose between ``the lease fires on its own''
and ``leaving the registry always releases ownership, however
triggered''; it has both.
...
\texttt{lease~c? = llapsed}, \texttt{TimedReap} (Section
\ref{sec:timedreap}) is enabled and \emph{also} depends on nothing but
\texttt{c?}'s own state -- not on any other connection existing, not on
any read ever running, not on any disconnect ever being detected. This is
...
chain
\texttt{TransportDies}\,;\,\texttt{LeaseLapse}\,;\,\texttt{TimedReap},
...
every scene it owned released, and Section~\ref{sec:verify} confirms this
...
(\texttt{Depart}, \texttt{TimedReap}, and \texttt{Claim}'s embedded sweep
of others) instead of one, plus one that must never remove anyone
...
paths instead of one: \texttt{Depart} and \texttt{TimedReap} each release
every scene \texttt{c?} owned in the same step they remove \texttt{c?}
(Sections~\ref{sec:depart} and~\ref{sec:timedreap} -- the two predicates
are identical apart from \texttt{TimedReap}'s added lease guard, so this
argument covers both at once), and \texttt{Claim}'s embedded sweep
...
meaningful discriminator elsewhere: a \texttt{TimedReap}-produced witness
\emph{also} requires nothing of transport state, so a witness confined to
\texttt{transport~c? = tup} could still, in principle, come from either
\texttt{Depart} or \texttt{TimedReap} acting on a connection that never
crashed at all -- an ordinary graceful departure or a lease that
genuinely lapsed from real inactivity, neither of which is a bug. What I6
rules out is narrower and sharper: that \texttt{Claim}'s own sweep, not
\texttt{Depart} or \texttt{TimedReap}, ever catches its \emph{own caller}
on the very call that caller is making -- the one mechanism H3 names, and
the one no legitimate departure operation is even attempting to perform
mid-write. In the fixed spec that cannot happen, because \texttt{Claim}'s
...
\texttt{Depart}, or \texttt{TimedReap} at all -- isolating the claim to
\texttt{Claim} alone, exactly as the fidelity-control discipline
requires.
...
operations that ever do -- \texttt{Depart}, \texttt{TimedReap}, and
...
\begin{enumerate}
  \item \textbf{Whichever fires first wins, and correctly.} If
    \texttt{TimedReap(c?)} and \texttt{Depart(c?)} are \emph{both} enabled
    in some state (a lapsed lease and a live, graceful-disconnect-eligible
    connection are not mutually exclusive conditions), firing either one
    removes \texttt{c?} from \texttt{registered} and releases every scene
    it owned -- the two predicates differ only in \texttt{TimedReap}'s
    extra guard, never in their effect. The second operation, whichever
  ...
  \item \textbf{A write is never stranded behind a timer, or vice versa.}
    ...
    and \texttt{TimedReap(c1)} for a lapsed \texttt{c1} are independent:
    neither's guard reads the other's parameter, and \texttt{Claim}'s own
    embedded sweep (Section~\ref{sec:claim}, step 2) would perform the
    identical release on \texttt{c1} if \texttt{TimedReap} had not already
    done so first. Whichever fires first, the reachable state after both
    have run (in either order) is the same: \texttt{c1} departed and
    released, \texttt{c2}'s claim resolved against a registry that no
    longer shows \texttt{c1} as a candidate to be swept. Reachability is
    monotonic under adding \texttt{TimedReap} to a spec that already had
    \texttt{Depart} and \texttt{Claim}: every state and trace reachable
    before this addendum remains reachable after it, because no existing
    operation's guard or effect changed.
  \item \textbf{A connection can never claim while being timed-reaped
    ...
    (Section~\ref{sec:claim}, step 1), and \texttt{TimedReap(c?)}
    requires \texttt{lease~c? = llapsed}. Because both are whole,
    indivisible steps, there is no state in which \texttt{Claim(c?, s?)}
    is ``in progress'' for \texttt{TimedReap(c?)} to observe a stale
    lease during: either \texttt{Claim} has already completed, in which
    case \texttt{c?}'s lease is \texttt{llive} and \texttt{TimedReap(c?)}
    is disabled, or it has not yet run at all.
\end{enumerate}

None of this needs a fourth invariant: it is I5 and I6, reread as claims
about \emph{any} pair of the four registry-mutating operations rather
than about \texttt{Claim} alone, and Section~\ref{sec:verify}'s full
model-check checks it the same way it checks everything else -- by
visiting every reachable state over the bounded carrier and confirming
no deadlock, which a genuine race manifesting as an inconsistent
intermediate state would show up as.
...
produced it: \texttt{Depart} and \texttt{TimedReap} compute the identical
postcondition (Section~\ref{sec:timedreap}), so a reachability goal alone
cannot certify that \texttt{TimedReap} specifically was exercised --
re-running the same goal at \texttt{DEFAULT\_SETSIZE 1}, which bounds
\texttt{CONN} to a single element and so rules out any second connection
appearing anywhere in the witness by construction of the carrier, still
returns a \texttt{Depart}-based trace, because \texttt{Depart}'s guard is
at least as permissive as \texttt{TimedReap}'s and ProB is free to choose
either once both are enabled:
...
\begin{verbatim}
  ...
  # *** TRACE (length=6):
  #  1: INITIALISATION(...)
  #  2: Connect(CONN1,IDENT1)
  #  3: Claim(CONN1,SCENE1)
  #  4: TransportDies(CONN1)
  #  5: LeaseLapse(CONN1)
  #  6: Depart(CONN1)
\end{verbatim}

What proves the unconditional strengthening is not a different goal but
the state this trace's own step 5 leaves behind: \texttt{LeaseLapse}'s
postcondition (Section~\ref{sec:leaselapse}) sets
\texttt{lease(CONN1) = llapsed} unconditionally, and \texttt{CONN1} is
still in \texttt{registered} (nothing in steps 1--5 removes it) --
exactly \texttt{TimedReap}'s guard, satisfied at that same point, by
inspection of the schema, with no further step needed to establish it.
\texttt{Depart(CONN1)} in step 6 is the trace ProB happened to print;
\texttt{TimedReap(CONN1)} was equally enabled in the identical state and
would have produced the identical \texttt{registered}/\texttt{owner}
values, because their predicates differ only in a guard both this state
already satisfies.
