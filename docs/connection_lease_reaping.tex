...
the strengthening this addendum makes: round 1's statement of I1 read
``nothing can indefinitely block \texttt{Depart} once it applies,'' which
depended on \emph{something} eventually triggering \texttt{Depart} or a
peer's \texttt{Claim} -- exactly the dependency the bead's own evidence
refutes (a connection persisting 25.79 hours with no peer ever writing or
disconnecting it). \texttt{TimedReap} removes that dependency: the chain
