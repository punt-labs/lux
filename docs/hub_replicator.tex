...
\texttt{Reconn} handles the dead-peer path.  It reconnects and re-queues
the scene that was in flight so nothing already drained is dropped; the
display's shown state is preserved, modelling a reconnect to a live display
whose rendered state survives a dropped socket.  If a deployment instead
finds the display process gone on this path (a fresh, empty display on
reconnect), this path must also re-mark every live scene, exactly as
\texttt{Ensure}/\texttt{Remark} do --- see the note in
...
\texttt{Reconn} models a reconnect to a display whose rendered state
survives a dropped socket, so it re-queues only the in-flight scene.  If a
deployment's \texttt{OSError} instead means the display process is gone ---
a fresh, empty display on reconnect --- then I4 requires this path to
re-mark every live scene, as the reap path does.  Model-checking I4 with
\texttt{Reconn} weakened to reset \texttt{displayed} without re-marking
makes the \texttt{Quiescent} violation reachable, which is the evidence for
that requirement.
