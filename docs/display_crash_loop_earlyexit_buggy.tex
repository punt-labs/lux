...
\begin{verbatim}
  fuzz -t docs/display_crash_loop_earlyexit_buggy.tex
\end{verbatim}
...
intermittent-crasher loop (exit before the crashers are caught, tally aged out each pass) as the counter-example trace:
\begin{verbatim}
...
The weakened \texttt{IsolExit} guard lets the worker leave isolation before
the intermittent crasher is caught; the window aging then wipes the tally,
so the crash count grows past the bound that the \texttt{STABLE\_INTERVAL}
$\geq$ \texttt{ATTRIBUTION\_WINDOW} tie enforces in the corrected design.
