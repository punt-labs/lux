%
% concept-extension-architecture.tex
%
% Concept: Extension System Architecture for Lux
% Explores how Lux could evolve from a fixed element vocabulary
% to a registry-based extension system. Written at concept stage;
% details will evolve if and when this work is prioritized.
%
% Compile: pdflatex concept-extension-architecture.tex
%
...
\begin{center}
  \fbox{\parbox{0.9\textwidth}{\centering\small
    \textbf{Not approved alternative concept.}
    ...
    implementation. See \texttt{docs/architecture/target/target.md} for the
    current target design.}}
\end{center}
\vspace{0.5em}

\title{Concept: Extension System Architecture for Lux}
\author{Exploring Registry-Based Element Dispatch and Self-Extension}
\date{April 2026 --- Concept Stage}
\hypersetup{
  pdftitle={Concept: Extension System Architecture for Lux},
  pdfauthor={Punt Labs},
  pdfsubject={Architecture Concept},
}
\maketitle
...
This document explores how Lux could evolve from a fixed element
vocabulary to a self-extending display server.  It sketches the
extension system, service API, and in-display agent SDK that would
make this possible.  This is concept-stage work only. It is not an
approved plan and is not under active development.

See also: \texttt{concept-self-extending-display.md} for the
high-level vision, and \texttt{concept-prfaq.tex} for the product
thinking.
...
\subsection{Relationship to Current Lux}

Lux currently has a fixed element vocabulary with a hardcoded ImGui
dispatcher.  This concept preserves the wire protocol shape, the
frame system, the ImGui render loop, and the MCP adapter --- but
refactors element dispatch from hardcoded to registry-based and adds
the extension system, service API, and in-display agent.
...
\quad Transport     & Unix socket (local) + TCP/TLS (remote).
  Length-prefixed JSON framing (inherited from current Lux). \\
...
\quad Frame Manager & Frame lifecycle, shared ownership, orphan model,
  dock bar, focus coordination.  Inherited from current Lux. \\
...
\ext{texture} &
  ...
  as ImGui image.  Limited interactivity
...
\ext{window} &
  ...
  Jupyter kernels, Pharo (via Postern).  Lifecycle tracked
  and focus coordinated but pixels not composited.
...
The concept proposes HTTP on localhost (default port 8423) as the
primary interface.  The shape follows Ollama: simple REST endpoints,
JSON payloads, streaming where appropriate.
...
static documentation.  An agent that has never seen this
...
The producer/consumer asymmetry is key: a small number of authors
produce extensions consumed by a larger number of users who never
need to author code themselves.
...
Extension installation follows the same consent pattern as current
Lux's render functions:

\begin{enumerate}[nosep]
  \item User or agent requests installation.
  \item Display shows the extension source code in a consent dialog.
...
If this concept is pursued, a phased migration preserves backward
compatibility:

\begin{enumerate}
  \item \textbf{Phase 1: Registry.}  Refactor element dispatch from
    hardcoded chain to registry lookup.  All existing element kinds
    become built-in extensions loaded at startup.  Wire protocol
    unchanged.  MCP tools unchanged.  Zero user-facing change.
...
product.  Phase 3 is where the self-extending concept becomes a
different product from current Lux.
