%
% display_crash_loop.tex
%
% A Z specification of the lux display crash-loop quarantine: a scene that
% is valid at the Hub but crashes the Display's renderer must not be re-fed
% to every respawned Display forever.  Companion to display_lifecycle.tex,
% which models the socket bind race among agents; this spec models the
% orthogonal concern of a poisonous scene looping one Display.
%
% The model covers the corrected attribution rule: every death is attributed
% to its suspect set (the whole batch in batching mode, a singleton in
% isolation mode), a scene is quarantined at THRESHOLD attributed deaths, and
% isolation is left only after a stable no-death interval -- long enough that
% an innocent's one batched increment ages out before another can land.  A
% crasher may be INTERMITTENT (it renders cleanly on some isolation probes);
% the model proves the loop still terminates and the innocent is never
% quarantined.
%
% Source of record (proposed by docs/architecture/display-crash-quarantine.md):
%   domain/hub/recovery.py    -- SendRecovery: excludes quarantined scenes
%                                from the post-crash re-mark; drives respawn
%   domain/hub/replicator.py  -- HubReplicator: batching/isolation send modes
%   domain/hub/crash_attribution.py -- per-scene tally, threshold, window,
%                                stable-interval exit, batched attribution
%   domain/hub/hub_display.py -- quarantined state and QuarantineRecord on the
%                                scene entry; excluded from replication
%
% Type-check:  fuzz -t docs/display_crash_loop.tex
...
%
% Formatting follows the fuzz house rules of the companion spec: \quad~ for
% continuation indent (never \t1..\t9), schema lines under 80 columns,
% predicates broken at \land/\lor/\implies with the operator starting the
% continuation line.
%
...
\title{The lux Display Crash-Loop Quarantine: a Z Specification}
\author{Formal model of the poison-scene quarantine rule}
\date{July 2026}
\maketitle
...
scenes suffice to exhibit a poison scene alongside an innocent one and are the
canonical check size; three exercise attribution against several crashers at once,
and Invariants~2 and~3 are re-confirmed at \texttt{DEFAULT\_SETSIZE 3} as well
(Section~\ref{sec:verify}).  The carrier is bounded by ProB's
...
In isolation mode each scene is probed alone.  The model gives probe
operations only to scenes in \texttt{crasher}: a probe of a scene outside
\texttt{crasher} changes nothing and threatens nothing, so it is abstracted
away as a stutter step rather than modelled --- the isolation loop in the
implementation does enumerate every live scene.  An intermittent crasher may crash
...
The guard \texttt{crasher $\subseteq$ quarantined} is a \emph{clock-free
abstraction} of the timed \texttt{STABLE\_INTERVAL} exit: rather than track wall
time, the model exits isolation exactly when nothing non-quarantined remains to
crash.  The two coincide only under the assumption that a crasher crashes at least
once per interval --- then a crash always resets the interval before it elapses,
so the timed exit fires only after the last crasher is quarantined, which is what
this guard encodes.  The abstraction is therefore \emph{strictly stronger} than
the timed exit: it cannot represent a scene that spaces its crashes beyond the
interval and so escapes un-quarantined (Invariant~2).  That escape is a tolerated
transient, not a loop, so leaving it out of the model costs no fidelity for the
property under test --- but it is why the bound is stated per episode, not
globally.
...
\paragraph{Invariant 2 --- crashes are bounded per episode (no fast respawn loop).}
...
across its clean renders, so its crashes still accumulate.

This bound is \emph{per isolation episode}, and it holds for exactly the scenes the
model represents: a crasher that crashes at least once per stable interval, so
that isolation is never left until the crasher is quarantined.  A scene that
spaced its crashes further apart than the stable interval would, in the timed
design, be left in isolation to render cleanly until the interval elapsed, exit
un-quarantined with its tally aged out, and crash again in a later episode from a
clean tally --- never quarantined, one crash per interval, unbounded over time.
That scene is \emph{deliberately tolerated as a transient} (see the design
document): the attribution window classifies crashes more than a window apart as
unrelated, so a rare crasher is indistinguishable from a string of independent
one-offs, and quarantining it would mean quarantining genuine transients.  It is
out of this model's scope by construction (see the note at \texttt{IsolExit}), and
it is a backoff-paced trickle, not a loop.  The property proved here is therefore
precisely: no respawn loop \emph{faster} than one crash per stable interval.
...
Invariants 2 and 3 are re-confirmed at the next carrier size, to check they do not
depend on there being only two scenes:
\begin{verbatim}
  # Invariants 2 and 3 at setsize 3 (must report: goal NOT found)
  probcli docs/display_crash_loop.tex -model_check -nodead \
    -goal "crashes>1+(THRESHOLD-1)*card(crasher)" \
    -p DEFAULT_SETSIZE 3 -p MAX_INITIALISATIONS 40
  probcli docs/display_crash_loop.tex -model_check -nodead \
    -goal "quarantined - crasher /= {}" \
    -p DEFAULT_SETSIZE 3 -p MAX_INITIALISATIONS 40
\end{verbatim}
