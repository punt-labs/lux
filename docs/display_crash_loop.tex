...
% orthogonal concern of a poisoned scene looping one Display.
%
...
%   domain/hub/crash_attribution.py -- per-scene death tally, threshold, window
...
%       -goal "crashes>THRESHOLD*card(poison)" -p DEFAULT_SETSIZE 2
%
...
The model attributes every crash to a single scene --- it starts, in effect, in
the design's isolation mode.  The implementation reaches isolation only after a
first \emph{batched} death that is unattributed (it is what triggers the switch),
so the implementation's worst case is \texttt{1 + THRESHOLD * \# poison}: the one
batched death plus the attributed deaths modelled here.  The model abstracts that
leading \texttt{+ 1} away because it does not change the property under test ---
the crash count is bounded either way --- and abstracting it keeps the model
focused on the quarantine rule that supplies the bound.
...
The loop never settles, either: with the quarantine clause struck, no operation
ever adds to \texttt{quarantined}, so it stays empty and a non-empty
\texttt{poison} can never become a subset of it.  The stable-state goal
\texttt{disp=up \& poison/=\{\} \& poison<:quarantined} is therefore unreachable
in the buggy variant by construction, not by a separate search --- the buggy
state space is infinite (\texttt{crashes} grows without bound), so it is the
directed search for \texttt{crashes > THRESHOLD * card(poison)} above, which halts
the instant the loop overshoots the bound, that is the runnable control.  That
