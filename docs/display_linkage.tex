...
Two further simplifications are implementer-relevant rather than merely
notational, and are called out here so neither is read as a claim about
where the real reset or the real wake happens.

\emph{The disconnected-backoff reset point.} This model resets
\texttt{discBackoff} to \texttt{0} at \texttt{RDialOk} --- the dial
succeeding --- because a dial and a send are one atomic step here.  Design
\S6.3 resets \texttt{\_disconnected\_backoff} at a \emph{clean send}
(\texttt{outcome == "clean"}), one step later than the dial in the real
two-phase cycle.  No invariant above is sensitive to which of the two
points is used: under a sustained disconnection \texttt{RDialOk} never
fires at all, so the reset in either place is equally inert while the
property I4 actually exercises (the climb) is underway.  The
\emph{implementation} must follow \S6.3's clean-send reset, not this
model's dial-success reset --- a dial that succeeds but whose immediate
send then fails should not reset the disconnected backoff, and this
model's coarser single-step cycle cannot distinguish that case from one
where the send also succeeds.

\emph{The wait/clear lost-wakeup window.} The real
\texttt{wait\_for\_reconnect} body is \texttt{woke = self.\_reconnected.wait(timeout);
self.\_reconnected.clear(); return woke} --- two statements, with a window
between the event's own wake and the unconditional \texttt{.clear()} in
which another thread could, in principle, call \texttt{.set()} again and
have that second signal silently discarded.  \texttt{RWaitWoken} and
\texttt{RWaitTimeout} each model the whole body as one atomic step, so
this window does not exist in the carrier and this model proves nothing
about it either way.  The consequence of a real lost wakeup here is
bounded --- the replicator does not learn of the second signal until its
\emph{next} dial attempt, which is bounded by the very backoff delay I4
already proves reaches its cap and no further, so this degrades to added
retry latency, never to a stuck wait or lost content.  It is out of this
model's scope by construction, not by oversight, and an implementer
reviewing the real \texttt{wait\_for\_reconnect} body should confirm that
bound holds of the shipped code.
...
\begin{description}
  ...
  \item[I6 --- the early-wake mechanism is present, reachable, and load-
    bearing.] The replicator's wait \emph{can} be broken by the shared
    event rather than only by the full delay elapsing, and a blocking
    sleep in place of that mechanism destroys the property.  ``Within one
    \texttt{DisplayLiveness} probe tick,'' the design's own real-time
    phrasing, is a timing claim an untimed model cannot discharge; see the
    invariant's own paragraph (\S\ref{sec:inv}) for exactly what is and is
    not proven.
...
it.  It is not, however, a pure passthrough on \texttt{dirty}.
\texttt{hub\_replicator.tex}'s own \texttt{Remark} and \texttt{Reconn}
both re-mark \emph{every live scene}, not merely the ones the failed
batch carried --- \texttt{dirty' = (dirty \cup drained) \cup \{s : SCENE
\mid store~s = vlive\}} in that spec's fuller carrier --- precisely
because the concrete recovery a heal performs is a fresh-process respawn
as often as a same-process reconnect: \texttt{Ensure} brings up a display
that shows nothing at all, so a live scene this model already believes
\texttt{shown}, but which the failed batch never touched, is not actually
on screen again until it is redelivered.  \texttt{RHeal} must perform the
same union here, or a heal silently strands exactly the scenes this
model's own \texttt{shown} set claims are safe --- the gap raised against
an earlier revision of this schema, which left \texttt{dirty} an
...
\where
  rphase = rhealing \\
  rphase' = ridle \\
  dirty' = dirty \cup live \\
  live' = live \\ shown' = shown \\ linked' = linked \\
  reconnected' = reconnected \\ wokenEarly' = wokenEarly \\
  reachable' = reachable \\ sendOk' = sendOk \\ regLock' = regLock \\
  discBackoff' = discBackoff \\ wedgeBackoff' = wedgeBackoff \\
  lInterval' = lInterval
...
\texttt{shown} is left unchanged, matching \texttt{Remark} and
\texttt{Reconn} alike, both of which leave \texttt{displayed} untouched on
the very same step: the correctness this union buys does not rest on
resetting the belief that a scene was delivered, only on forcing every
live scene back into the owed set regardless of that belief, so the next
cycle redelivers it whether the old belief happened to be right
(\texttt{Reconn}'s stale-but-correct case) or wrong (\texttt{Ensure}'s
freshly-emptied one) --- \texttt{hub\_replicator.tex} proves its own I4
exactly this way, ``because of that re-mark, not because of any reset.''
Because the union only ever adds scenes to \texttt{dirty} and never
removes one, it cannot weaken I1; because it never writes \texttt{linked},
it is invisible to I2's own claim about the \texttt{linked} transition,
addressed directly in that invariant's own paragraph below.
...
\texttt{DisplayLiveness}'s own \texttt{\_probe()} does not stop at the
dial.  \texttt{self.\_clients.get().ping(self.\_ping\_timeout) is not
None} is a dial immediately followed by a round-trip health check ---
structurally the replicator's own \texttt{RDialOk}-then-\texttt{RSendFail}
shape, not a single test.  Folding a ping failure into \texttt{reachable}
would repeat the exact confusion \S1 diagnoses for the replicator itself:
\texttt{reachable} is the environment's truth about whether a fresh dial
would succeed \emph{right now}, and a display can answer a fresh dial
perfectly well while the client \texttt{\_probe()} already holds is
wedged or has just dropped its socket --- a send-time fact, independent
of the dial-time one, exactly the distinction \S\ref{sec:state} draws for
the replicator's own \texttt{sendOk}.  \texttt{check\_once()} treats two
consecutive failed probes as a disconnection regardless of \emph{which}
of the two ways \texttt{\_probe()} failed: it calls
\texttt{self.\_clients.drop()} --- the real counterpart of
\texttt{linked' = clear} --- and switches to
\texttt{\_DISCONNECTED\_PROBE\_INTERVAL} pacing.
\texttt{LDialOkPingFail} models exactly the ping-fails-despite-a-live-dial
case, reading \texttt{sendOk} rather than \texttt{reachable} for the same
reason \texttt{RSendFail} does:

\begin{schema}{LDialOkPingFail}
  \Delta Link
\where
  regLock = free \\
  reachable = set \\
...
No reconcile fires here: the display this step drops never held the
content long enough for that to matter, so \texttt{dirty} is left exactly
as it was, to be reconciled properly at the next successful dial from
either actor, by \texttt{RDialOk} or \texttt{LDialOk} in the ordinary way
--- the same argument \texttt{RHeal}'s own paragraph above makes for why
a heal that never touches \texttt{linked} needs no reconcile of its own.
None of the three liveness-actor operations is guarded by \texttt{rphase}:
the prober runs regardless of whatever cycle the replicator is in, which
is the whole point of it being a genuinely independent second actor
rather than a second phase of the same worker.
...
Restricted to the connect transition itself, \texttt{dirty' = dirty \cup
live} under the guard \texttt{linked = clear} fires only inside
\texttt{RDialOk} and \texttt{LDialOk} --- i.e.\ only on the step that
turns the registry from disconnected to connected.  Because \texttt{linked}
is one shared boolean, at most one of the two dialing operations can be
the step that performs that transition for a given disconnection;
whichever one does not (because it finds \texttt{linked} already
\texttt{set}) takes the other branch, which leaves \texttt{dirty}
untouched by that reconcile.  The reconcile cannot be missed either,
because it is not a separate pass triggered \emph{after} the transition:
it is part of the same atomic step that performs the transition.

\texttt{RHeal} performs the identical union \texttt{dirty' = dirty \cup
live}, but unconditionally, from \texttt{rhealing}, with \texttt{linked}
passed through unchanged --- \texttt{set} throughout, by construction:
\texttt{RSendFail} and \texttt{RHeal} both leave \texttt{linked}
untouched, so a heal is never, in this model, also a disconnect.  The two
unions are disjoint by guard alone: \texttt{RDialOk}/\texttt{LDialOk}'s
reconcile requires \texttt{linked = clear} beforehand; \texttt{RHeal}'s
requires \texttt{rphase = rhealing}, reachable only through
\texttt{RSendFail} with \texttt{linked = set} throughout.  No reachable
state satisfies both guards at once, so the two reconciles never race,
never double-fire for the same event, and never substitute for each
other --- they guard different hazards.  I2's own name is deliberately
scoped to the first, the connect-triggered reconcile that gives the
invariant its title; the second, \texttt{RHeal}'s heal-triggered re-mark
against the risk of a heal stranding content this model already believed
delivered, is a distinct guarantee, corroborated separately in
\S\ref{sec:verify}.

A held scene believed already shown is the sharpest witness for either
mechanism: only a reconcile (\texttt{RDialOk}/\texttt{LDialOk} under
\texttt{linked = clear}) or a heal (\texttt{RHeal}) can put a scene into
\texttt{dirty} without first taking it out of \texttt{shown}; every other
producer of \texttt{dirty} (\texttt{PushContent}) removes it from
\texttt{shown} in the same step.
...
\paragraph{I6 --- the early-wake mechanism is present, reachable, and
load-bearing.}
\[
  \Diamond\,(wokenEarly = set)
\]
is reachable: the replicator's wait \emph{can} be broken by the shared
event, not only by the full delay elapsing.  \S Fidelity shows this is
exactly the property that a raw blocking sleep --- \texttt{RWaitWoken}
deleted --- makes unreachable, reproducing ``up-to-full-delay blindness''
precisely: the mechanism this proves present is also the mechanism whose
absence is observably worse.

This is not the whole of design \S12's I6, and the gap is worth stating
plainly rather than papering over.  The design's own words are that a
reconnect breaks the replicator out ``within one \texttt{DisplayLiveness}
probe tick,'' a real-time bound.  \texttt{RWaitTimeout}'s only guard in
this model is \texttt{rphase = rwaitdisc}: at a state where
\texttt{reconnected = set}, both \texttt{RWaitWoken} and
\texttt{RWaitTimeout} are enabled, and the model admits a legal trace that
takes the timeout exit \emph{despite} the wake signal being available.  An
untimed Z model has no clock against which ``within one probe tick'' could
even be stated, let alone proved; reachability of
\texttt{wokenEarly = set} shows the early-exit path exists and matters, but
it does not show the replicator \emph{always} takes it the instant the
signal is available.  The actual real-time promptness claim rests on two
facts this model proves true by construction and one it does not model at
all: I8 below establishes that the wait never holds the registry lock, so
the prober's own dial is never blocked from setting the signal; \S6.4's
5-second \texttt{\_DISCONNECTED\_PROBE\_INTERVAL} bounds how long the
prober can go between attempts (a real-time fact about the implementation,
not a Z proposition); and the actual scheduling fairness between
\texttt{RWaitWoken} and \texttt{RWaitTimeout} at the moment both are
enabled is a property of the real thread scheduler, not of this
specification. A liveness variant of this property --- ``if parked with
the signal set, the wait is broken by the wake and not by repeated,
signal-ignoring timeouts'' --- is expressible as an LTL check under weak
fairness on \texttt{RWaitWoken}; it was considered and set aside for this
revision (\S Fidelity) as not adding proof strength proportionate to its
cost, since the honest restatement above already draws the line the design
document itself needed redrawn.
...
The scope of this claim is worth stating precisely, not left implicit.
\texttt{regLock = held} is unreachable in \emph{every} state of the
corrected model, not merely in \texttt{rwaitdisc} --- every dial
(\texttt{RDialOk}, \texttt{RDialFail}, \texttt{LDialOk}, \texttt{LDialFail})
is one atomic Z step, so nothing this model can observe ever holds the
lock across more than the single step in which it is momentarily
acquired and released.  ``The wait never holds the lock'' is therefore
true here in the degenerate sense that the lock is never held across
\emph{any} multi-step span at all; the specific hazard I8 is guarding
against --- a wait that wrongly extends the hold across the whole parked
interval --- is exercised only by fidelity control (c), which is the one
place \texttt{regLock} does anything interesting in this specification.
This is an honest, not an evasive, degeneracy: it reflects that
\texttt{ClientRegistry.get()}'s real critical section genuinely is short
and bounded, and the design's actual risk was always specifically about
\emph{extending} that hold across \texttt{wait\_for\_reconnect}, which is
exactly what fidelity control (c) targets.

This paragraph's deadlock-freedom claim is also scoped to
\texttt{regLock} alone.  \texttt{hub\_replicator.tex} proves its own I3
--- no thread ever holds its store lock (\texttt{StoreLock}, guarding
\texttt{HubDisplay}'s content) and the client send lock at once --- for
the \emph{single-actor} replicator model; this specification does not
re-derive a two-lock ordering across \texttt{regLock} and
\texttt{StoreLock} for the two-actor case, and rests on a premise it
states rather than proves: \texttt{wait\_for\_reconnect} acquires no
second lock while parked. \texttt{ClientRegistry.\_lock} and
\texttt{StoreLock} are two independent locks in the real implementation,
and nothing in \texttt{wait\_for\_reconnect}'s own body (\S6.3) touches
\texttt{StoreLock} at all --- but that is a fact about the implementation
this model takes as a premise, not a conclusion this model's operations
derive, because \texttt{StoreLock} does not appear in \texttt{Link}'s
carrier. An implementation review should confirm that premise holds of
the shipped code, not merely of this specification.
...
Two further mechanisms, added to close a fidelity gap raised in review
(\S\ref{sec:inv}'s I2 paragraph and the \texttt{DisplayLiveness} paragraph
above), are corroborated the same way --- reachable, not dead code, even
though neither is one of the eight named invariants in its own right:
\begin{verbatim}
  # RHeal corroboration -- a heal that must reconcile more than the
  # failed batch (some other live scene already believed shown) is
  # reachable (must report: goal FOUND)
  probcli docs/display_linkage.tex -model_check -nodead \
    -goal "rphase = rhealing & card(live \ dirty) > 0" -p DEFAULT_SETSIZE 2

  # LDialOkPingFail corroboration -- the liveness actor's own send-time
  # (ping) failure, distinct from a dial-time failure, is reachable
  # (must report: goal FOUND)
  probcli docs/display_linkage.tex -model_check -nodead \
    -goal "reachable = set & sendOk = clear & regLock = free" \
    -p DEFAULT_SETSIZE 2
\end{verbatim}
The first goal is the precise witness for the defect raised against an
earlier revision of \texttt{RHeal}: a heal entered while some live scene
is already believed \texttt{shown} and is \emph{not} part of the failed
batch --- exactly the state in which the old, passthrough \texttt{RHeal}
would have left that scene unreconciled.
