...
\begin{center}
  \fbox{\parbox{0.9\textwidth}{\centering\small
    \textbf{Archived 2026-07-03 --- superseded; retained for history.}
    This document is no longer maintained and does not describe the current
    system. It captured a Layer-2 integration model that predates the
    Hub/Display architecture. For design intent see
    \texttt{docs/architecture/target/target.md}; for the current architecture
    see \texttt{docs/architecture/system.tex}. See
    \texttt{docs/archive/README.md} for why this was archived.}}
\end{center}
\vspace{0.5em}
...
Archived. This is a formal integration specification retained for historical
reference only; it is not the canonical rewrite-target document and does not
describe the current system. For the rewrite target, start with
\texttt{target/target.md}. For the current whole-system narrative, see
\texttt{system.tex}.
