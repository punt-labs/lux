...
This is a formal subsystem specification of the \emph{legacy single-process
display server} --- the design where one process owns the socket, the scene
state, and the render loop. It is not the shipped three-process Hub/Display
runtime the system is migrating toward, and it is not the canonical
rewrite-target document. For the rewrite target, start with
\texttt{target/target.md}; for the current whole-system narrative --- including
the Hub/Display slice that supersedes this single-process model --- see
\texttt{system.tex}. This spec is retained because its refinement and partition
tests (\texttt{tests/test\_display\_refinement.py},
\texttt{tests/test\_display\_partition.py}) still hold the current display code
...
A client sends a new scene, replacing any existing one. An empty element set is
a removal, not an install: the scene disappears (\texttt{hasScene' = zfalse})
rather than lingering as an empty husk --- the Hub blanks a removed scene by
re-pushing it with no elements, and the display treats that empty push as the
dismissal it is. A non-empty push installs the scene as before. Either way the
event queue is cleared, since old element references are stale.
...
\begin{schema}{ReceiveScene}
  ...
  (newElemIds? \neq \emptyset \implies hasScene' = ztrue) \\
  (newElemIds? = \emptyset \implies hasScene' = zfalse) \\
  sceneId' = newSceneId? \\
  elemIds' = newElemIds? \\
