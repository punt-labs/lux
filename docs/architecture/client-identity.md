...
**Status:** delivered design record. The operator ratified this document's
open questions on 2026-07-28 (see [Ratification](#ratification)), and the
identity work it drove shipped as **DES-057** in `DESIGN.md` (the identity
...
compact entry does not itself carry. Read [target/target.md](target/target.md)
...
Ratification of this document is recorded in `DESIGN.md` as **DES-057**
(the identity train, PRs #290-#292) — see the status note at the top of
this document for how the shipped shape diverged from the plan below.
