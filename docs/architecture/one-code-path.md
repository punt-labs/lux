...
1. The MCP tools hold the logic. `tools.py` is 673 lines across 23 tools; the
...
with the three `show*` conveniences composing the one `render` operation.

| Today's tool | Operation | State model |
|---|---|---|
| `show` | `render` | Hub-owned (`HubDisplay`) |
| `show_table` | `render` (adapter builds the tree) | Hub-owned |
| `show_dashboard` | `render` (adapter builds the tree) | Hub-owned |
...
The operations layer must not become a second 673-line god module. It is split
...
proxied-versus-owned call that carries real scope, because it moves state
across the boundary and touches the replicator; it is called out in the open
...
actually returns. The typed design removes the possibility of drift: one
Pydantic model is both the operation's result type and the schema the surfaces
derive. A payload that the model accepts cannot be rejected by a schema built
from the model.
...
| `POST /menus/items` | `register_menu_item` | `MenuItem` | `Ok \| OpError` |
...
class UpdateRequest(BaseModel):
    patches: list[dict[str, object]]   # id + set/remove; validated in the op
...
change it builds a request and formats a result.
...
def show(
    ...
    request = RenderRequest(
        ...
        frame=FrameSpec(
            ...
            size=tuple(frame_size) if frame_size else None,
            flags=FrameFlags(**frame_flags) if frame_flags else None,
            layout=frame_layout,
        )
        if any((frame_id, frame_title, frame_size, frame_flags, frame_layout))
        else None,
    )
    return _format_scene(OPERATIONS.render(request, scope=_scope()))
```

`_format_scene` maps `SceneShown` to `"shown:<id>"` and `OpError` to
...
`layout` now fails when `RenderRequest` is constructed, and `_format_scene`
turns that validation error into the same `"error: …"` string. `show_table` and
`show_dashboard` are also adapters — they build an element tree and call the
same `render` operation, exactly as they do today, but through the operation
rather than by re-calling the tool.

The `_scope()` helper resolves the caller's connection scope. For an MCP tool
it is the session's `ConnectionId` from the connection, as today.
...
mode, so this is a proven shape in the family; luxd adopts it. The deprecated
WebSocket route and its `websocket_server` import are deleted, and the
...
luxd binds `127.0.0.1` by default but accepts a `--host` argument. A
loopback-fixed transport policy then rejects every off-loopback handshake, so an
operator who sets `--host 0.0.0.0` gets a listener that accepts the TCP
connection and then refuses every request. That is a silent-looking trap: the
bind succeeds and every call fails.

The resolution for this unit is to refuse a non-loopback `--host` at startup
with a clear message, rather than bind it and reject each request. Remote access
...
onto that. And the log ordering is tidied in the same pass: today a rejected
handshake logs "MCP WebSocket connected" before the guard fires, which reads as
an accepted connection that then vanishes; the connection is logged after it is
accepted, not before the guard runs.
...
These are the forks the design cannot settle alone. Each carries a
recommendation for the ratification gate.

**1. Menu state — proxied or Hub-owned?** Every other display-fact operation is
proxied, but menus are agent-submitted UI, which the Hub owns everywhere else.
Owning menu state makes the writes plain Hub writes and the read
Hub-authoritative, at the cost of moving state across the boundary and touching
the replicator. *Recommendation: own it in the Hub.* It is the consistent
answer — submitted UI is Hub-owned — and it removes the last menu reach-around.
It lands in PR B as its own commit so the scope is contained.

**2. REST caller identity and scope.** An MCP call carries a `ConnectionId` from
its session, which scopes its owned scenes and subscriptions. A REST call has no
such session. For the local single-user slice this does not bite: the code is
already a single-connection slice today, where `clear` is global and scenes
share one scope. *Recommendation: REST uses one default Hub scope for this unit,
matching today's single-connection reality; per-caller REST scoping is deferred
with the multi-user future.* When multiple users arrive, a REST call carries an
explicit session identity — a header or a field — and the same scoping the MCP
leg already has.

**3. mcp-proxy retirement timing across repos.** Retiring the bridge from lux's
path is a lux change plus a Claude Code config change, and it does not require
mcp-proxy itself to change. But the plugin that ships lux's MCP configuration
has to switch from the proxy command to the direct HTTP URL, and that switch is
what users pick up. *Recommendation: land the streamable-HTTP server (PR E) and
verify Claude Code connecting directly before changing the shipped plugin
config, so the direct path is proven live before it becomes the default.* Keep
the proxy path working until the direct path is confirmed, then flip the config
in a follow-on release. This is sequencing, not a compatibility shim — no dual
readers in the code, only a config value that changes once.
...
logic in a 673-line `tools.py`. `lux show beads` wrote the display socket
