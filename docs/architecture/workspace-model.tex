...
This is a formal workspace-model specification of a shipped subsystem, not the
canonical rewrite-target document. The workspace model it describes --- frames,
the World menu, and the dock bar --- has shipped since roughly v0.9 and is live
in the current display. It predates the Hub/Display split, so it models the
workspace layer within the legacy single-process design; the target topology
(\texttt{target/target.md}) relocates authoritative state into the Hub without
retiring these workspace concepts. For the current whole-system narrative, see
\texttt{system.tex}.

The workspace model, inspired by Pharo/Squeak's live environment, makes
the display a \emph{workspace} containing independent, movable
\emph{frames} (ImGui inner windows).  Each frame is owned by a connected
client and contains one or more \emph{scenes}.
...
focus on the workspace state --- frames, ownership, and the World menu.
...
Frame visibility state.  \texttt{fvClosed} is a value here, not the
absence of the frame: the user putting a window away says nothing about
what it holds, so a closed frame keeps its scenes, its owner and its
place in \texttt{frames}, and is simply not painted (DES-088).
\begin{zed}
  FrameVis ::= fvNormal | fvMinimized | fvClosed
\end{zed}
...
The user clicks the close button on a frame.  It is a \emph{visibility}
transition and nothing else (DES-088): the frame stays in \texttt{frames}
with its owner and its scenes, so a later \texttt{RestoreFrame} has
something to act on and a later push into it still reads as a repeat
rather than an arrival.  No interaction event is sent to the owning
client --- where a window sits is the Display's own business, and the
event that used to carry a close is retired.
...
\begin{schema}{CloseFrame}
  ...
  \where
  frameId? \in frames \\
  frameVis~frameId? \in \{ fvNormal, fvMinimized \} \\
  frameVis' = frameVis \oplus \{ frameId? \mapsto fvClosed \} \\
  frames' = frames \\
  frameOwner' = frameOwner \\
  frameScenes' = frameScenes \\
  sceneFrame' = sceneFrame \\
...
% -------------------------------------------------------------------
\subsection{Dispose Frame (Content)}
% -------------------------------------------------------------------

The removal \texttt{CloseFrame} used to be, now under its own name and
reached only by a \emph{content} event: an empty push, a manifest purge
that disowns the last scene, a frame TTL, or Clear All.  The client is
saying the content is gone, so the frame goes with it --- whatever
visibility the user had left it in.  A frame with no content is a husk,
and a closed one is no exception, which is what keeps a closed frame from
being an unbounded leak.
...
Enabled from \emph{any} visibility, which is what makes a closed frame
reachable again: an applet's menu-entry click, a dock pill, Expand All,
or the Windows menu's list of closed frames all arrive here (DES-088).
...
\begin{schema}{RestoreFrame}
  ...
  \where
  frameId? \in frames \\
  frameVis~frameId? \in \{ fvMinimized, fvClosed \} \\
