...
% regardless of a changed name. W12 (lux-833p) is the dedicated z-spec
% bead that re-models invariant 2 ("at most one live connection per
% HubId") against the corrected HubId-keyed logic, generalized from one
% Hub to N concurrently-reconnecting Hubs; it cannot run until W8/W9/W11
% have all merged. Until W12 lands, treat every clause below that scopes
% preemption by name (HI4 above all) as a historical record of the
% pre-W11 code, not an authoritative description of the shipped
% behaviour -- do not hand-edit this model to patch it; that re-model is
% W12's scope.
% ------------------------------------------------------------------------
%
...
\textbf{SUPERSEDED --- pending W12 (\texttt{lux-833p}) re-model.} As of
...
for no longer reflect the shipped code. W12 will re-model invariant 2
(``at most one live connection per \texttt{HubId}'') against the
corrected \texttt{HubId}-keyed logic once W8/W9/W11 have all merged.
Read this document as a historical record of the pre-W11 design until
then, not as a description of current behaviour.}}
\vspace{1em}
