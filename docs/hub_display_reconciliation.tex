...
%
% ------------------------------------------------------------------------
% SUPERSEDED -- PENDING W12 (lux-833p) RE-MODEL
%
% As of W11 (lux-u9gb, merged), Hub-connection preemption keys on the
% connecting Hub's HubId (hostname+pid) -- ClientRegistry.hub_fd_for /
% SocketListener.hub_fd_for / HubReconciliation._preempt_stale_hub --
% never on the declared ConnectMessage.name. This model still represents
% a Hub connection's identity as a bare CONNECTION_ID with no name/HubId
% distinction, and HI4 in hub_display_reconciliation_coverage.md ("two
% kind="hub" identifies for two different declared names -- neither
% preempts the other -- preemption scopes to the shared name") states the
% PRE-W11 rule: it no longer reflects the code. Two connections sharing a
% name but declaring distinct HubIds now correctly coexist (the bug W11
% fixes -- every production Hub declares the identical hardcoded name), and
% a reconnect under the SAME HubId still preempts its predecessor
% regardless of a changed name. W12 (lux-833p) HAS landed:
% docs/crosshost_hubid_preemption.tex re-models invariant 2 ("at most one
% live connection per HubId") against the corrected HubId-keyed logic,
% generalized from one Hub to N concurrently-reconnecting Hubs, with a
% name-keyed fidelity control that reproduces the double-owner defect this
% document's name-keyed clauses describe. Treat every clause below that
% scopes preemption by name (HI4 above all) as a historical record of the
% pre-W11 code; crosshost_hubid_preemption.tex is authoritative for
% invariant 2 -- do not hand-edit this model to patch it.
% ------------------------------------------------------------------------
%
...
\textbf{SUPERSEDED --- see \texttt{crosshost\_hubid\_preemption.tex}
(W12, \texttt{lux-833p}).} As of
W11 (\texttt{lux-u9gb}, merged), single-owner preemption is keyed on the
connecting Hub's \texttt{HubId} (hostname + pid), never on the declared
\texttt{ConnectMessage.name}. This model still treats a connection's
identity as a bare \texttt{CONNECTION\_ID} with no name/\texttt{HubId}
distinction, so \textbf{HI4} (\texttt{hub\_display\_reconciliation
\_coverage.md}: ``two \texttt{kind="hub"} identifies for two different
declared names -- neither preempts the other -- preemption scopes to the
shared name'') and the \texttt{hubOwner}-keyed-by-name shape it stands in
for no longer reflect the shipped code. W12 has landed:
\texttt{crosshost\_hubid\_preemption.tex} re-models invariant 2
(``at most one live connection per \texttt{HubId}'') against the
corrected \texttt{HubId}-keyed logic, generalized to N Hubs, and is
authoritative for that invariant. Read this document as a historical
record of the pre-W11 design, not as a description of current behaviour.}}
\vspace{1em}
