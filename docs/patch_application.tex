...
%                                            field check (LOGS and SKIPS per
%                                            patch) then
%                                            apply_set under
%                                            try/except (ValueError, TypeError)
%   src/punt_lux/display/replica/element_walk.py     -- find: locate or None; a missing
%                                            id is warned and skipped
%   src/punt_lux/domain/element_abc.py     -- apply_patch: snapshot + rollback
%                                            (atomic multi-field set)
%   src/punt_lux/display/replica/scene_replica.py:207      -- apply_update -> applier.apply
%   src/punt_lux/display/render_loop.py:681     -- _handle_update: the message loop
%                                            self._scene_manager.apply_update(msg)
%                                            inside a last-resort try/except that
%                                            logs and still acks (residual guard)
%
...
\texttt{Element.apply\_patch}; round~4 by surfacing an unknown field per
patch --- logging and skipping it --- rather than letting it raise out of
\texttt{\_apply\_patch\_set} and kill the loop.  All four are now fixed;
each survives only as a fidelity variant (Section~\ref{sec:fidelity}) that
reintroduces the defect to confirm the model still detects it.  Empirical
case-by-case patching kept missing cases, so this document models the
boundary as a finite state machine over a bounded batch, letting ProB
enumerate every patch class exhaustively.
...
\paragraph{Round 4 --- crash on unknown-field structural error.}
This defect --- since fixed by surfacing the unknown field per patch, logging
and skipping it --- is reproduced by making \texttt{ApplySurface} replace
\texttt{live' = live} with \texttt{live' = terminated}, modelling a field-name
check that raises before the value-rejection \texttt{try} and propagates
through \texttt{apply\_update} to the message loop.  Invariant~CF's goal
\texttt{live=terminated} is then \emph{found}, and so is Invariant~BC's goal
