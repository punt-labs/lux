...
| [writer_publish_generation.tex](./writer_publish_generation.tex) | — | Session-writer publish atomicity (`lux-wk3p` review): the `_writer` publish path lands a message only in the inbox generation live at put time. Invariant WG — no cross-generation put: neither an orphan-put into a queue detached by a concurrent `drop_session` (silent loss) nor a departed session's message delivered into a same-`connection_id` reconnect's fresh inbox (cross-session leak). The fix mirrors `offer`'s D1 atomicity — `_writer`'s lookup-or-create and put under one `_inboxes_lock` hold. One fidelity control: [split](./writer_publish_generation_split_buggy.tex) (the release-then-put window at the real code's lock boundary) reproduces both the orphan-loss and cross-generation witnesses |
