...
Four properties must hold across every reachable state.  None is placed in
\texttt{Sys}, for the reason given with that schema.
...
The full reachable state space is also enumerated without a goal, which is
the deadlock check --- \texttt{DisposeFrame} returns every id to its unseen
pool, so the system cycles rather than terminating:
\begin{verbatim}
  probcli docs/frame-visibility-lifecycle.tex -model_check \
    -p DEFAULT_SETSIZE 2
\end{verbatim}

\paragraph{Status of the ProB leg.}  At the time of writing, \texttt{probcli}
is not installed on the development host: the \texttt{Makefile}'s
\texttt{prob} target resolves \texttt{\$(HOME)/Applications/ProB/probcli},
and that directory holds only a 314-byte \texttt{probcli.zip} that
\texttt{unzip} rejects as not an archive.  The \texttt{fuzz} leg has been run
and passes.  The model-check commands above are therefore \emph{specified and
unexecuted}, and the traces in Section~\ref{sec:fidelity} are hand-derived
rather than ProB-produced.  This is recorded rather than glossed: the model is
type-correct and the fidelity argument is explicit, but nobody has yet watched
ProB find those traces.
...
specification with today's behaviour restored in exactly two places ---
...
\texttt{ConsumeFocus} are identical.  All four goals above must be
\emph{found} there.

\paragraph{Bug A --- the frame the user closed cannot be raised.}
\begin{enumerate}
  \item \texttt{PushNewFrame}$(s_1, f_1)$: the applet installs its board.
    \texttt{frames} $= \{f_1\}$, \texttt{vis}~$f_1 = vOpen$.
  \item \texttt{Close}$(f_1)$ --- in the buggy variant this removes $f_1$
    from \texttt{frames} and $s_1$ from \texttt{content}, while the ghost
    records \texttt{userClosed} $= \{f_1\}$.
  \item \texttt{Raise}$(f_1)$ is now \emph{not enabled}: its precondition
    $f_1 \in frames$ fails.  The user's menu click reaches a frame that is
    not there, which is what \texttt{raise\_frame} reports as
    \texttt{raised: false}.
  \item The state satisfies \texttt{userClosed} $\not\subseteq$
    \texttt{frames}, violating Invariant~3 --- goal found.
\end{enumerate}
Under this specification step~2 leaves $f_1$ in \texttt{frames} with
\texttt{vis}~$f_1 = vClosed$, so step~3 is enabled and restores it.

\paragraph{Bug B --- the closed frame reopens on a background push.}
\begin{enumerate}
  \item \texttt{PushNewFrame}$(s_1, f_1)$: voxd installs its music scene.
  \item \texttt{Close}$(f_1)$ --- buggy: $f_1$ leaves \texttt{frames}, $s_1$
    leaves \texttt{content}, \texttt{userClosed} $= \{f_1\}$.
  \item The track changes and voxd pushes the same scene into the same frame.
    Because $s_1 \notin content$, this is \texttt{PushNewFrame}$(s_1, f_1)$
    again, not \texttt{PushRepeat}: $f_1$ is recreated \texttt{vOpen}, and the
    buggy variant additionally sets \texttt{focus} $= \{f_1\}$ and
    \texttt{autoFocused} $= \{f_1\}$.
  \item Now $f_1 \in frames$, $f_1 \in userClosed$ and \texttt{vis}~$f_1 =
    vOpen$: Invariant~4 violated --- goal found.  \texttt{autoFocused}
    $\neq \emptyset$ violates Invariant~1 in the same state.
\end{enumerate}
Under this specification step~2 keeps $s_1$ in \texttt{content}, so step~3 is
\texttt{PushRepeat} --- a $\Xi$ operation --- and the frame stays closed.
That difference, between a push that reads as new and a push that reads as a
repeat, is the whole of bug B, and it is created by the close, not by the
push.

\paragraph{The tab steal.}
\begin{enumerate}
  \item \texttt{PushNewFrame}$(s_1, f_1)$, then
    \texttt{PushNewScene}$(s_2, f_1)$ --- buggy: \texttt{activeTab}~$f_1 =
    s_2$ and \texttt{tabStolen} $= \{f_1\}$.
  \item Invariant~2 violated --- goal found.
\end{enumerate}
This is the third affordance DES-060 gated to new scenes, and R8 retires it
with the other two: a second scene arriving in a frame the user is reading
must not pull the selection off the tab they chose.

\paragraph{What the difference establishes.}  The reachability searches
return \emph{goal not found} for this specification and \emph{goal found} for
the control, on the same carriers and the same initial state.  The model is
therefore not too abstract to see the defects: it distinguishes exactly the
designs that exhibit them from the design that does not.
