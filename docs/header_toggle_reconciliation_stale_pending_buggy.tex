%
% header_toggle_reconciliation_stale_pending_buggy.tex
%
...
% the design closes becomes reachable.  It models a fix that renders the pending value but never lets the Hub
% take it back, so the Display can disagree with the Hub permanently.
%
% The edit:
% %   Repush keeps pending' = pending instead of clearing it to nopending.
%
% Must find:
% %   probcli header_toggle_reconciliation_stale_pending_buggy.tex -model_check \
%     -nodead -goal "pending/=nopending & clicked=zfalse" -p DEFAULT_SETSIZE 2
%   probcli header_toggle_reconciliation_stale_pending_buggy.tex -model_check \
%     -nodead -goal "rendered=ztrue & clicked=zfalse & shown/=hub" \
%     -p DEFAULT_SETSIZE 2
%
...
\title{The lux Collapsing-Header Toggle Discipline: Fidelity Control (b), the Stale Pending}
