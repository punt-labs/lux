...
\begin{faqpair}{What evidence do we have that customers want this?}\label{faq:evidence}
  ...
  The honest gap: this evidence is \emph{internal}, and the ``customer'' is
  currently the company itself. External adoption is zero --- no usage data, no
  retention figures, no third-party testimonials. In the four-risks framing, a
  customer that is the company is a High Value-risk signal, and this document
  rates it as such. Every ``value'' claim here should be read against that
  boundary (see~\faqref{faq:next-step} and the Value risk in the Risk
  Assessment).
\end{faqpair}
...
Value      & \textcolor{RiskRed}{High} &
  All demonstrated value is \emph{internal}, and the ``customer'' is currently
  the company itself --- one primary human developer plus his agent fleet
  (see~\faqref{faq:evidence}). ``Runs in every session'' counts agent sessions,
  not distinct humans; external adoption is zero. In the four-risks framework a
  customer that is the company is a High Value-risk signal, so this row is rated
  High, not Medium. It is why \faqref{faq:next-step} makes the day-7 external
  trial the single most important next step. \\
