...
\begin{center}
  ...
  {\large\color{AccentGray}
    ...
    natively with GPU-accelerated Dear~ImGui on the developer's own machine,
    with no vendor compute} \\[0.3em]
  {\large\color{AccentGray}
    ...
    agent through the GUI, from a fixed, curated element set that composes
    without limit}
\end{center}
...
many independent agents, one developer, and no shared place to watch them.
...
a small set of ImGui widgets does. This is the core idea. Lux is not an IDE for
agents; it prescribes no screens and no workflows. It is a medium through which
an agent composes whatever UI it needs, so agents communicate through arbitrary
interfaces built from one fixed vocabulary. Second, and the direction Lux is
heading, an
...
\begin{faqpair}{How is this different from terminal scrollback, Claude Desktop, or vendor dashboards?}\label{faq:competition}
  ...
  on the developer's machine. It is not a chat interface, not a cloud console,
  and not an IDE for agents. Lux prescribes no screens or workflows; it lets
  agents communicate through arbitrary interfaces of their own composition. It is
  a screen the agents share: one window, one frame per agent, live. See
  \faqref{faq:competitors} for the detailed competitive analysis.
\end{faqpair}
...
\begin{faqpair}{What can Lux render today, and can agents ask me questions?}\label{faq:capabilities}
  ...
  Sessions and apps also own entries in the shared menu bar (and the matching
  World menu): each entry belongs to the live session that registered it, and a
  click is serviced instantly by that session's own running process. Completing
  coverage of every ImGui widget as a built-in element is the v2
...
\begin{faqpair}{What evidence do we have that customers want this?}\label{faq:evidence}
  Lux is released (v0.22 on PyPI as \texttt{punt-lux} and on the plugin
  ...
  sessions, not just tests. A second proof point now exists at the application
  level: vox (the companion voice tool) ships a music player as a Lux Hub app --
  its daemon registers a menu entry, receives the user's clicks over its
  persistent connection, and drives playback -- accepted end-to-end in a live
  demo (a pixel click producing audio). Still Punt Labs' own tool, so still
  internal evidence, but it validates the app surface, not just the agent one.
...
\begin{faqpair}{What is the architecture?}\label{faq:architecture}
  ...
  \begin{itemize}[nosep]
    \item \textbf{\texttt{luxd}}: the session Hub and the engine. It decodes
      wire elements, holds authoritative UI state, ownership, and handler
      dispatch in \texttt{HubDisplay}, pushes element replicas to the display,
      re-dispatches interactions that come back from clicks, and serves every
      client surface -- MCP (streamable HTTP), REST, and the CLI -- through one
      typed operations facade, so a capability runs the same engine code
      whichever surface it entered from.
    ...
    \item \textbf{\texttt{lux mcp-serve}}: the per-session MCP server the
      Claude Code plugin spawns. It forwards the session's MCP traffic to
      \texttt{luxd} verbatim and holds a persistent listening connection on a
      background thread, which is what lets the session own menu entries whose
      clicks are answered instantly -- by a running process, never by a turn of
      the model.
  \end{itemize}
  ...
  This lets the tiers sit on different machines. The Display connects to
  the Hub over a Unix socket; agents and apps connect to the Hub over HTTP and
  WebSocket on localhost (127.0.0.1:8430) by default. A remote agent works
  today by tunneling that localhost port over SSH. The practical payoff:
...
\begin{faqpair}{What is the v1-to-v2 roadmap, and what is the migration risk?}\label{faq:roadmap}
  ...
  The element-migration risk this section used to carry is retired: all 25
  element kinds are on the distributed Element-ABC path with two-tier remote
  handler dispatch, and the legacy render path is deleted. The remaining
  v2 feasibility exposure is the rendering layer's test gap -- the renderer is
  verified by eyes and introspection tools, not by automated visual regression
  -- and the interaction vocabulary itself: richer GUI-driven work needs more
  input primitives and the composites that arrange them, each a
  protocol-plus-renderer change with an evaluator gate (see the Risk
  Assessment).
\end{faqpair}
...
\begin{faqpair}{If Lux succeeds, what breaks first?}\label{faq:scaling}
  ...
  \begin{enumerate}[nosep]
    ...
    \item \textbf{Renderer maintainability debt.} \texttt{display/server.py} is
      roughly 1,500 lines, well over the 300-line target. Decomposition is in
      progress (the original monolith was split once; the legacy renderer
      modules died with the element migration). This does not break at
...
\begin{faqpair}{What are the key metrics for success?}\label{faq:metrics}
  \begin{enumerate}[nosep]
    ...
    \item \textbf{Hub-app adoption}: applications built on the Hub's client
      surface beyond the agent tools (one today: vox's music player), tracking
      whether the app surface pulls its own builders.
    \item \textbf{Interaction usage}: sessions that use the ask-the-user loop
      and the menu surface, distinguishing Lux-as-display from
      Lux-as-interaction-surface.
  \end{enumerate}
...
\begin{faqpair}{What are we explicitly not building?}\label{faq:wont}
  ...
  no in-display LLM authoring agent. Its client surfaces (MCP, REST, CLI,
  library) bind to the developer's own machine: there is no hosted service, no
  public web API, and no account. It has one rendering mode (native ImGui), not
  several. It does not host local model inference, does not render to the web or
  Electron, and does not target mobile or tablet. These exclusions
  are what keep the built-in vocabulary coherent and the trust model simple; the
  full list is in the Feature Appendix. One earlier exclusion was reversed:
  applets are now in scope as \emph{Lux applets} -- programs on the library
  client that own menu entries and service clicks (the vox music player is the
  first third-party one; \texttt{lux-beads} the first bundled one).
\end{faqpair}
...
Feasibility & \textcolor{RiskAmber}{Medium} &
  The core is shipped and works: Hub/Display, the MCP and REST front doors, the
  data-display core, the interaction loop (dialogs, buttons, session menus with
  instant click servicing), and formally-verified lifecycle and atomicity
  models. The element migration that used to be the primary exposure is
  complete: 25 of 25 kinds on the Element-ABC path, legacy path deleted. The
  remaining exposure is the rendering layer's automated-test gap (correctness
  there is verified by introspection tools and human demos, not visual
  regression in CI) and the ordinary risk of growing the interaction
  vocabulary (see~\faqref{faq:roadmap}). \\
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  ...
  \featureitem{MCP front door}{a first-class entry point: \texttt{luxd} serves
    MCP over streamable HTTP, and each Claude Code session runs
    \texttt{lux mcp-serve}, whose persistent connection lets the session own
    menu entries with instantly-serviced clicks}\label{feat:mcp}
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  ...
  \featureitem{Hosted service API}{the MCP, REST, CLI, and library surfaces are
    thin local clients of one engine on the developer's machine; Lux does not
    become a hosted web service, expose a public API, or require an
    account}\label{feat:no-http}
  ...
  \featureitem{Applets as arbitrary in-display applications}{what IS in scope
    (reversing this row's earlier full exclusion, by operator ruling
    2026-08-01): \emph{Lux applets} -- external programs on the library client
    that own menu entries and render through the Hub, either shipped by their
    own tool (vox's music player) or bundled with Lux for software that is not
    Punt Labs-built (\texttt{lux-beads}; top/clock/irc-class candidates). What
    stays out: applications running \emph{inside} the display process, and Lux
    itself becoming an application platform with in-display code
    execution}\label{feat:no-apps}
